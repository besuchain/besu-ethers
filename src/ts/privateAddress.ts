...
export function getPrivateAddress(privateAddress: string): string {
  if (typeof privateAddress !== 'string') {
    logger.throwArgumentError(
      'invalid private address. Not a string',
      'privateAddress',
      privateAddress
    );
  }
  ...
  if (privateAddress.match(RegEx.bytes)) {
    ...
    if (privateAddress.length === 66) {
      ...
      return logger.throwArgumentError(
        `invalid hexadecimal encoded private address. Length ${privateAddress.length} and not 66`,
        'privateAddress',
        privateAddress
      );
    }
  ...
  if (privateAddress.match(RegEx.base64)) {
    if (privateAddress.length === 44) {
      ...
      return logger.throwArgumentError(
        `invalid base64 encoded private address. Length ${privateAddress.length} and not 44`,
        'privateAddress',
        privateAddress
      );
    }
  ...
  return logger.throwArgumentError(
    'invalid private address. No hexadecimal or base64 encoded',
    'privateAddress',
    privateAddress
  );
}
