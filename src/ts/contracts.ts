...
function resolveAddresses(
  ...
  if (paramType.baseType === 'array') {
    ...
    return Promise.all(
      value.map((v) => resolveAddresses(signerOrProvider, v, paramType.arrayChildren))
    );
  }
...
function runMethod(contract: Contract, functionName: string, options: RunOptions): RunFunction {
  ...
  return function (...params): Promise<any> {
    ...
    if (
      params.length === method.inputs.length + 1 &&
    ...
    return resolveAddresses(contract.signer || contract.provider, params, method.inputs).then(
      (params) => {
...
export class Contract {
  ...
  constructor(
    ...
    if (Signer.isSigner(signerOrProvider)) {
      ...
      logger.throwError('invalid signer or provider', Logger.errors.INVALID_ARGUMENT, {
    ...
    if (this.provider) {
      ...
      try {
        defineReadOnly(
          this,
          'addressPromise',
          Promise.resolve((<any>this.interface.constructor).getAddress(addressOrName))
        );
      } catch (error) {
    ...
    Object.keys(this.interface.functions).forEach((name) => {
      ...
      if (this.populateTransaction[name] == null) {
        defineReadOnly(
          ...
          name,
          runMethod(this, name, { transaction: true })
        );
      }
