...
providerNode1.on('debug', (info) => {
  console.log(
    `Sent "${info.action}" action to node 1 with request: ${JSON.stringify(
      info.request
...
providerNode2.on('debug', (info) => {
  console.log(
    `Sent "${info.action}" action to node 2 with request: ${JSON.stringify(
      info.request
    )}\nResponse: ${JSON.stringify(info.response)}`
  );
});
...
providerNode3.on('debug', (info) => {
  console.log(
    `Sent "${info.action}" action to node 3 with request: ${JSON.stringify(
      info.request
    )}\nResponse: ${JSON.stringify(info.response)}`
  );
});
...
describe('Ethers', () => {
  ...
  test('signed transaction matches EEA client', async () => {
    ...
    const wallet = new PrivateWallet(privateKey);
  ...
  describe('getPrivateTransactionReceipt', () => {
    test('missing hash', async () => {
      const result = await providerNode1.getPrivateTransactionReceipt(
  ...
  describe('Create privacy group and send transactions', () => {
    ...
    const deployData =
      '';
    const eeaWallet = new PrivateWallet(privateKey);
    ...
    describe('pre transaction checks', () => {
      describe('get private transaction count from', () => {
        test('node 3', async () => {
          prePrivateNonce = await providerNode3.getPrivateTransactionCount(
            txFromAddress,
            testPrivacyGroupOptions
          );
        ...
        test('node 2', async () => {
          const result = await providerNode2.getPrivateTransactionCount(
            txFromAddress,
            testPrivacyGroupOptions
          );
        ...
        test('node 1', async () => {
          const result = await providerNode1.getPrivateTransactionCount(
            txFromAddress,
            testPrivacyGroupOptions
          );
          expect(result).toEqual(prePrivateNonce);
    ...
    describe('Post transaction count checks', () => {
      describe('get private transaction count from', () => {
        test('node 3 from address', async () => {
          const result = await providerNode3.getPrivateTransactionCount(
            txFromAddress,
            testPrivacyGroupOptions
          );
    ...
    `(
      ...
      ({ testDescription, txFromAddress, privacyGroup, txOptions }) => {
        ...
        test('get private transaction count from node1', async () => {
          privateNonce = await providerNode1.getPrivateTransactionCount(
            txFromAddress,
            privacyGroup
          );
        ...
        test('get private transaction counts from each node', async () => {
          const nonceNode1 = await providerNode1.getPrivateTransactionCount(
            txFromAddress,
            privacyGroup
          );
          expect(nonceNode1).toBeGreaterThanOrEqual(privateNonce + 1);

          const nonceNode2 = await providerNode2.getPrivateTransactionCount(
            txFromAddress,
            privacyGroup
          );
          expect(nonceNode2).toBeGreaterThanOrEqual(privateNonce + 1);

          const nonceNode3 = await providerNode3.getPrivateTransactionCount(
            txFromAddress,
            privacyGroup
          );
