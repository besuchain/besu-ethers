...
export class PrivateWallet extends Wallet implements PrivateSigner {
  ...
  //   - sendTransaction
  populatePrivateTransaction(
    transaction: PrivateTransactionRequest
  ): Promise<PrivateTransactionRequest> {
    return resolveProperties(this.checkTransaction(transaction)).then((tx) => {
      ...
      if (tx.nonce == null) {
        tx.nonce = this.provider.getPrivateTransactionCount(this.getAddress(), transaction);
      }
      ...
      if (tx.from == null) {
        ...
        tx.from = Promise.all([this.getAddress(), this.provider.resolveName(tx.from)]).then(
          (results) => {
